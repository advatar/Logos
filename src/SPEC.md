...
- Rust `protocol-core`: a sorted binary Merkle tree (`protocol-core::merkle`) over `Hash32` with leaf/node domain separation, used by `RegistryState::membership_root()` and `revocation_root()`. Both roots are bound into the post public-inputs hash alongside `threshold_public_key_hash` per §5. Non-membership proofs against the revocation root are deferred until the RISC0 guest fixes the in-circuit encoding (sparse-Merkle vs. indexed-Merkle).
- Rust `lp0016-registry`: the LEZ registry program with `create_forum` / `register_member` / `slash_member` instructions and `ForumState` / `MemberRecord` / `RevocationRecord` / `StakePolicy` account types. SPEL annotations are doc-style today (`// #[lez_program]` etc.) and become real macros under the `spel` feature flag once `logos-scaffold` is installed. The hand-written `src/registry/idl/lp0016_registry.json` is the source-of-truth for the on-chain interface; the `logos-scaffold build idl` output must match it.
...
- Rust `protocol-core`: still **dev** — `MockZkReceipt` stand-in for the RISC0 receipt (the structural plumbing for `Risc0 { receipt_bytes, image_id }` is ready in `STATUS.md → Phase 4`) and a trusted-dealer DKG (`DealerShares::trusted`).
