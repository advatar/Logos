...
pub struct ForumConfig {
    ...
    pub k: u8,
    pub n: u8,
    pub moderators: Vec<ModeratorId>,
...
impl AnonymousPostEnvelope {
    pub fn new_dev(
        ...
        let ciphertext_hash = digest(
            ...
            &[&forum.forum_id, &post_id, &share.x.to_be_bytes(), &share.y.to_be_bytes()],
        );
        let retro_tag = crate::retro_tag(&forum.forum_id, &member.coeffs, &content_id, &nonce);
        let k_bytes = [forum.k];
        let public_inputs_hash = digest(
            "proof-public-inputs",
            &[&forum.forum_id, &k_bytes, &content_id, &nonce, &ciphertext_hash, &share_commitment, &retro_tag],
        );
